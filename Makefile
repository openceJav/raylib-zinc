...
#------------------------------------------------------------------------------------------------
PROJECT_NAME          ?= retro_maze_3d
PROJECT_VERSION       ?= 1.0
PROJECT_BUILD_MODE    ?= RELEASE

# TODO: Support PROJECT_BUILD_PATH? At this moment project is build in src directory (where resources are located)
...
ifeq ($(PLATFORM),PLATFORM_DESKTOP)
    ifeq ($(PLATFORM_OS),WINDOWS)
        ...
        LDFLAGS += retro_maze_3d.rc.data
        # -Wl,--subsystem,windows hides the console window
        ifeq ($(PROJECT_BUILD_MODE), RELEASE)
...
ifeq ($(PLATFORM),PLATFORM_WEB)
    ...
    LDFLAGS += -s USE_GLFW=3 -s TOTAL_MEMORY=67108864 -s FORCE_FILESYSTEM=1 --preload-file resources

    # Define a custom shell .html and output extension
    LDFLAGS += --shell-file $(RAYLIB_INCLUDE_PATH)/shell.html
    EXT = .html
endif
ifeq ($(PLATFORM),PLATFORM_RPI)
